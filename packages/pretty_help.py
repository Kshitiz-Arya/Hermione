...
def command_help(docstring: str):
    ...
    command_format, example = rest.split('Example:\n')

    return (discription, args, command_format, example)
...
class Paginator:
    ...
    def add_cog(self, title: Union[str, commands.Cog],
        ...
        embed = self._new_page(page_title,
                               (title.description or "") if cog else "")
    ...
    def add_command(self, command: commands.Command):
        ...
        page = self._new_page(
            ...
            f"{self.prefix}{desc}{self.suffix}" or "",
        )

        page.add_field(name="Arguments",
                       value=f"{self.prefix}{args}{self.suffix}",
                       inline=False)

        page.add_field(name="Command Format",
                       value=f"{self.prefix}{form}{self.suffix}",
                       inline=False)

        page.add_field(name="Example",
                       value=f"{self.prefix}{example}{self.suffix}",
                       inline=False)
    ...
    def add_group(self, group: commands.Group,
                  commands_list: List[commands.Command]):
        """
        ...
        page = self._new_page(
            group.name,
            f"{self.prefix}{self.__command_info(group)}{self.suffix}" or "")
...
class PrettyHelp(HelpCommand):
    ...
    def __init__(self, **options):
        ...
        self.color = options.pop(
            "color",
            discord.Color.from_rgb(randint(0, 255), randint(0, 255),
                                   randint(0, 255)),
        )
    ...
    async def send_bot_help(self, mapping: dict):
        ...
        async with channel.typing():
            ...
            help_filtered = (filter(lambda c: c.name != "help", bot.commands)
                             if len(bot.commands) > 1 else bot.commands)
            ...
            sorted_map = sorted(
                ...
                if isinstance(cg[0], commands.Cog) else str(cg[0]),
            )
